        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                Varyings vert(Attributes IN, uint instanceID : SV_InstanceID)
                {
                    ...
                    half directDiffuse = abs(dot(normalWS, mainLight.direction)); //half lambert, to fake grass SSS
                    //fog
                ...
                half4 frag(Varyings IN) : SV_Target
                {
                    ...
                    shadow = min(mainLight.shadowAttenuation, shadow);
                    return half4(IN.color * (0.1 + shadow * 0.9), 1);
                }
                ENDHLSL
            }
            ...
            Pass
            {
                ...
                half4 frag(Varyings IN) : SV_Target
                {
                    ...
                    Light mainLight = GetMainLight(TransformWorldToShadowCoord(IN.positionWS));
                    shadow = min(mainLight.shadowAttenuation, shadow);
                    return half4(mainLight.shadowAttenuation, 1, 1, 1.0);
                    //return half4(IN.color * (0.1 + shadow * 0.9), 1);
                }
                ENDHLSL
            }
